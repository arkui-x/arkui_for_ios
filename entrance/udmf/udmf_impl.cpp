...
std::vector<std::string> UdmfClientImpl::GetPlainTextRecords(const RefPtr<UnifiedData>& unifiedData)
{
    ...
    LOGI("GetSinglePlainTextRecord, records size: %{public}zu", records.size());
    for (const auto& record: records) {
        UDMF::UDType type = record->GetType();
        LOGI("GetSinglePlainTextRecord, type: %{public}d", type);
        if (type == UDMF::UDType::PLAIN_TEXT) {
            ...
            LOGI("GetSinglePlainTextRecord, str: %{public}s", str.c_str());
