    ...
    textureId:(int64_t)textureId abilityInstanceId:(int32_t)abilityInstanceId
{
    if (self = [super init]) {
        ...
        [self.callMethodMap setObject:[callSetTextureSize copy] forKey:[self method_hashFormat:@"setTextureSize"]];
...
- (void)dealloc
{
    LOGI("AceTexture dealloc instanceId=%{public}lld", self.instanceId);
}
