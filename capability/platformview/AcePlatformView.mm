...
- (NSString *)updatelayout:(NSDictionary *)params
{
    ...
    @try {
        ...
        UIView* platformView = [embeddedView view];
        NSString* xid = [embeddedView getXComponentID];
...
- (void)initPixelBuffer
{
    ...
    CVReturn status = CVPixelBufferCreate(kCFAllocatorDefault,
                                          ...
                                          &self->_textureOutput);
    if(status == kCVReturnSuccess && self.textureOutput != nil){
        NSLog(@"AcePlatformView: initPixelBuffer status: %d",status);
    }
}
...
- (CVPixelBufferRef _Nullable)copyPixelBuffer
{
    ...
    CVPixelBufferLockBaseAddress(self.textureOutput, 0);
    void *pxdata = CVPixelBufferGetBaseAddress(self.textureOutput);
    
    if(pxdata != nil){
        CGColorSpaceRef rgbColorSpace = CGColorSpaceCreateDeviceRGB();
        NSUInteger bytesPerRow = CVPixelBufferGetBytesPerRow(self.textureOutput);
        ...
        CVPixelBufferUnlockBaseAddress(self.textureOutput, 0);
    }
